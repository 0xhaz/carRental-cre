...
function readCampaignInfo(runtime2, evm, vehicleId) {
  const callData = encodeFunctionData({
    abi: paymentProtocolAbi,
...
function submitReport(runtime2, evm, vehicleId, action) {
  ...
  evm.writeReport(runtime2, {
    receiver: runtime2.config.campaignReceiverAddress,
    report: report2,
    gasConfig: { gasLimit: "1000000" }
  }).result();
  runtime2.log(`  -> ${action} report submitted for vehicle #${vehicleId}`);
}
function getMockCampaigns(now) {
  ...
  return [
    {
      ...
      campaign: {
        ...
        currentAmount: 3n * ETH,
        deadline: now - 5n * ONE_DAY,
    ...
    },
    {
      ...
      campaign: {
        targetAmount: 5n * ETH,
        currentAmount: 4n * ETH,
        deadline: now + 10n * ONE_DAY,
...
var onCronTrigger = (runtime2) => {
  runtime2.log("=== RegShield Campaign Monitor ===");
  const evm = new ClientCapability(SEPOLIA_CHAIN_SELECTOR);
  const now = BigInt(Math.floor(runtime2.now().getTime() / 1000));
  let simulationMode = false;
  const campaigns = [];
  try {
    const vehicleIds = readActiveCampaigns(runtime2, evm);
    runtime2.log(`Active campaign vehicles: ${vehicleIds.length}`);
    for (const vehicleId of vehicleIds) {
      try {
        const campaign = readCampaignInfo(runtime2, evm, vehicleId);
    ...
    simulationMode = true;
    runtime2.log("No live chain data — running in SIMULATION MODE with mock campaigns");
    campaigns.push(...getMockCampaigns(now));
    runtime2.log(`Loaded ${campaigns.length} mock campaigns for simulation`);
  }
  if (campaigns.length === 0) {
    runtime2.log("No active campaigns. Idle.");
  ...
  for (const { vehicleId, campaign } of campaigns) {
    if (campaign.isFinalized)
      continue;
    const fundingPercentage = campaign.targetAmount > 0n ? campaign.currentAmount * 100n / campaign.targetAmount : 0n;
    runtime2.log(`  Vehicle #${vehicleId}: ${fundingPercentage}% funded, deadline=${campaign.deadline}, ` + `investors=${campaign.investorCount}, cancelled=${campaign.isCancelled}`);
    if (campaign.isCancelled && campaign.investorCount > 0n) {
      runtime2.log(`  Campaign cancelled with ${campaign.investorCount} investors — triggering batch refund`);
      if (!simulationMode) {
        submitReport(runtime2, evm, vehicleId, "CAMPAIGN_CANCELLED");
      } else {
        runtime2.log(`  -> [SIM] Would submit CAMPAIGN_CANCELLED for vehicle #${vehicleId}`);
      }
      reportsSubmitted++;
      continue;
    }
    if (campaign.deadline > 0n && campaign.deadline < now) {
      if (fundingPercentage < campaign.minFundingPercentage) {
        runtime2.log(`  Deadline passed! ${fundingPercentage}% < ${campaign.minFundingPercentage}% min — campaign FAILED`);
        if (!simulationMode) {
          submitReport(runtime2, evm, vehicleId, "CAMPAIGN_FAILED");
        } else {
          runtime2.log(`  -> [SIM] Would submit CAMPAIGN_FAILED for vehicle #${vehicleId}`);
        }
        reportsSubmitted++;
      } else {
        runtime2.log(`  Deadline passed but funding met (${fundingPercentage}%) — campaign successful`);
      }
    } else {
      const daysLeft = campaign.deadline > 0n ? Number((campaign.deadline - now) / 86400n) : -1;
      runtime2.log(`  Campaign active, ${daysLeft >= 0 ? `${daysLeft} days left` : "no deadline set"}`);
    }
  }
