...
function readVehicleInfo(runtime2, evm, tokenId) {
  const callData = encodeFunctionData({
    abi: vehicleNftAbi,
...
function encodeComplianceReport(action, actionData) {
...
function submitReport(runtime2, evm, action, actionData, label) {
  ...
  evm.writeReport(runtime2, {
    receiver: runtime2.config.complianceReceiverAddress,
    report: report2,
    gasConfig: { gasLimit: "500000" }
  }).result();
  runtime2.log(`  -> ${label} report submitted`);
}
function getMockVehicles(now) {
  ...
  return [
    {
      meta: {
        ...
        color: "White",
        mileage: 32000n,
        registrationExpiry: now - 30n * ONE_DAY,
    ...
    },
    {
      meta: {
        ...
        insuranceExpiry: now - 10n * ONE_DAY
      },
    ...
    },
    {
      ...
      info: { maintenanceCount: 5n, incidentCount: 2n, lastMaintenanceDate: now - 120n * ONE_DAY }
    }
  ];
}
var onCronTrigger = (runtime2) => {
  runtime2.log("=== RegShield Compliance Monitor ===");
  ...
  try {
    ...
    mocks.forEach((v, i2) => vehicles.push({ id: BigInt(i2 + 1), ...v }));
  ...
  for (const { id, meta, info } of vehicles) {
    runtime2.log(`Vehicle #${id}: ${meta.make} ${meta.model}`);
    if (meta.registrationExpiry > 0n && meta.registrationExpiry < now) {
      runtime2.log(`  Registration expired at ${meta.registrationExpiry}`);
      ...
      if (!simulationMode) {
        submitReport(runtime2, evm, Action.SUSPEND_VEHICLE, actionData, "SUSPEND_VEHICLE (expired registration)");
      } else {
        runtime2.log(`  -> [SIM] Would submit SUSPEND_VEHICLE (expired registration)`);
      }
      reportsSubmitted++;
      continue;
    }
    if (meta.insuranceExpiry > 0n && meta.insuranceExpiry < now) {
      runtime2.log(`  Insurance expired at ${meta.insuranceExpiry}`);
      const actionData = encodeAbiParameters([
        { name: "vehicleId", type: "uint256" },
        { name: "reason", type: "string" }
      ], [id, "Insurance expired — detected by CRE compliance monitor"]);
      if (!simulationMode) {
        submitReport(runtime2, evm, Action.SUSPEND_VEHICLE, actionData, "SUSPEND_VEHICLE (expired insurance)");
      } else {
        runtime2.log(`  -> [SIM] Would submit SUSPEND_VEHICLE (expired insurance)`);
      }
      reportsSubmitted++;
      continue;
    }
    if (info.lastMaintenanceDate > 0n && now - info.lastMaintenanceDate > MAINTENANCE_OVERDUE_THRESHOLD) {
      const daysSince = (now - info.lastMaintenanceDate) / (24n * 60n * 60n);
      runtime2.log(`  Maintenance overdue: ${daysSince} days since last service`);
      const actionData = encodeAbiParameters([
        { name: "vehicleId", type: "uint256" },
      ...
      if (!simulationMode) {
        submitReport(runtime2, evm, Action.RECORD_MAINTENANCE, actionData, "RECORD_MAINTENANCE (overdue)");
      } else {
        runtime2.log(`  -> [SIM] Would submit RECORD_MAINTENANCE (overdue by ${daysSince} days)`);
      }
      reportsSubmitted++;
    }
  }
