...
var NHTSA_API_BASE = "https://vpic.nhtsa.dot.gov/api/vehicles/decodevin";
function readTotalPayments(runtime2, evm) {
...
function readMilestoneStatus(runtime2, evm, paymentId) {
  const callData = encodeFunctionData({
    abi: paymentProtocolAbi,
...
function readVehicleMetadata(runtime2, evm, vehicleId) {
  const callData = encodeFunctionData({
    abi: vehicleNftAbi,
...
function readVehicleInvestmentTotal(runtime2, evm, vehicleId) {
  const callData = encodeFunctionData({
    abi: paymentProtocolAbi,
...
function readVehicleStatus(runtime2, evm, vehicleId) {
  const callData = encodeFunctionData({
    abi: vehicleNftAbi,
...
function decodeVIN(runtime2, http, vin) {
  const fetchVIN = (requester, vinCode) => {
  ...
  return http.sendRequest(runtime2, fetchVIN, consensusIdenticalAggregation())(vin).result();
}
function encodeReport(paymentId, milestoneName) {
  return encodeAbiParameters([
    { name: "paymentId", type: "uint256" },
    { name: "milestoneName", type: "string" }
  ], [paymentId, milestoneName]);
}
function submitMilestone(runtime2, evm, paymentId, milestoneName) {
  ...
  evm.writeReport(runtime2, {
    receiver: runtime2.config.paymentReceiverAddress,
    report: report2,
